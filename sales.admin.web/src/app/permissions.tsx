...
export function SalesPermissionsProvider({ children }: PropsWithChildren) {
  const { token } = useAdminAuth()
  const [data, setData] = useState<SalesPermissionsResponse | null>(null)
  ...
  useEffect(() => {
    ...
    setIsLoading(true)
    fetchIdentityJson<SalesPermissionsResponse>('/me/permissions/sales', { token })
      .then((resp) => {
        if (!isActive) return
        setData(resp)
      })
...
export function useSalesPermissions() {
  ...
  if (!ctx) throw new Error('useSalesPermissions must be used within SalesPermissionsProvider')
  return ctx
}
...
export function PermissionGate({ permission, anyPermissions, policyOverride, children }: PermissionGateProps) {
  const { can, canAny, uiPolicy, isLoading } = useSalesPermissions()

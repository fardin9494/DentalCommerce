...
export function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true)
    try {
      await fetchJson<void>('/sales/auth/check', { token: pwd })
      setToken(pwd)
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="w-full max-w-sm bg-white border rounded-lg shadow-sm p-6">
        <h1 className="text-lg font-semibold mb-4 text-center">ورود به پنل فروش</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <p className="text-[11px] text-gray-500 text-center mt-2">
            پسورد فقط در مرورگر شما نگهداری می‌شود و به‌صورت هدر روی درخواست‌ها فرستاده می‌شود.
          </p>
        </form>
      </div>
    </div>

...
function getFriendlyError(err: unknown, context: 'otp-request' | 'otp-verify' | 'password-login') {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-xl bg-white border rounded-2xl shadow-sm p-6 md:p-8">
        <div className="mb-6 text-center">
          <h1 className="text-xl font-semibold text-slate-900">ورود به پنل ادمین فروش</h1>
          <p className="mt-1 text-xs text-slate-500">برای مدیریت سفارش‌ها و گزارش‌های فروش وارد حساب خود شوید.</p>
        </div>
              ...
              {otpStep === 'phone' ? (
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="w-full bg-slate-900 text-white rounded py-2 text-sm hover:bg-slate-800 disabled:opacity-60"
                  >
                    {isVerifyingOtp ? 'در حال تایید...' : 'تایید کد و ورود'}
                  </button>
                  <button
                    type="button"
                    onClick={requestOtp}
                    disabled={isRequestingOtp || !phone.trim()}
                    className="w-full border border-slate-300 rounded py-2 text-sm hover:bg-slate-50 disabled:opacity-60"
                  >
                    {isRequestingOtp ? 'در حال ارسال مجدد...' : 'ارسال مجدد کد'}
                  </button>
                </div>
              )}
            </form>

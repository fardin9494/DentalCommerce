...
/// <summary>
/// LIFO (Last In First Out): تخصیص بر اساس تاریخ ورود - جدیدترین موجودی اول خروج می‌کند
/// </summary>
public sealed class AllocateTransferLineLifoHandler
    : IRequestHandler<AllocateTransferLineLifoCommand, IReadOnlyList<TransferAllocationDto>>
{
    private readonly IInventoryDbContext _db;
    private readonly ITransactionRunner _tx;

    public AllocateTransferLineLifoHandler(IInventoryDbContext db, ITransactionRunner tx)
    {
        _db = db;
        _tx = tx;
    }

    public async Task<IReadOnlyList<TransferAllocationDto>> Handle(AllocateTransferLineLifoCommand req, CancellationToken ct)
    {
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            try
            {
                result = await _tx.ExecuteAsync(async ct =>
                {
                    ...
                    // Save existing segments before clearing
                    ...
                    // LIFO: Order by CreatedAt descending (newest first)

...
type InventoryPermissionsResponse = {
...
export function InventoryPermissionsProvider({ children }: PropsWithChildren) {
  const { token } = useAdminAuth()
  const [data, setData] = useState<InventoryPermissionsResponse | null>(null)
  ...
  useEffect(() => {
    ...
    setIsLoading(true)
    fetchIdentityJson<InventoryPermissionsResponse>('/me/permissions/inventory', { token })
      .then((resp) => {
        if (!isActive) return
        setData(resp)
      })
...
export function useInventoryPermissions() {
  ...
  if (!ctx) throw new Error('useInventoryPermissions must be used within InventoryPermissionsProvider')
  return ctx
}
...
export function PermissionGate({ permission, anyPermissions, policyOverride, children }: PermissionGateProps) {
  const { can, canAny, uiPolicy, isLoading } = useInventoryPermissions()

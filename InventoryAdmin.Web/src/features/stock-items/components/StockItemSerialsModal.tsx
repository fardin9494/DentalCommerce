    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-3xl max-h-[90vh] rounded-xl bg-white shadow-xl flex flex-col">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Serials</h3>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {isLoading ? (
            ...
            <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center">
              <p className="text-sm text-red-700">{(error as Error).message || 'Failed to load serials.'}</p>
            </div>
          ) : serials.length === 0 ? (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-6 text-center">
              <p className="text-sm text-slate-600">No available serials found.</p>
            </div>
          ) : (
            <div className="space-y-3">
              <div className="text-sm text-slate-600">
                Count: <span className="font-medium text-slate-900">{serials.length.toLocaleString('fa-IR')}</span>
              </div>

...
public sealed record GetPricingPermissionsQuery(Guid UserId) : IRequest<PricingPermissionDto>;

public sealed class GetPricingPermissionsHandler : IRequestHandler<GetPricingPermissionsQuery, PricingPermissionDto>
{
    private readonly IIdentityDbContext _db;
    private readonly IClock _clock;
    private readonly AdminOptions _adminOptions;

    public GetPricingPermissionsHandler(IIdentityDbContext db, IClock clock, AdminOptions adminOptions)
    {
        _db = db;
        _clock = clock;
    ...
    public async Task<PricingPermissionDto> Handle(GetPricingPermissionsQuery req, CancellationToken ct)
    {
        ...
        var items = PricingPermissions.All
            .Select(p => new PricingPermissionItemDto(
                p.Key,
                p.Title,
                p.Description,
        ...
        return new PricingPermissionDto(req.UserId, isSuper, uiPolicy, items);
    }
}

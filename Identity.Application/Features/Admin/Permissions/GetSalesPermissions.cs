...
public sealed record GetSalesPermissionsQuery(Guid UserId) : IRequest<SalesPermissionDto>;

public sealed class GetSalesPermissionsHandler : IRequestHandler<GetSalesPermissionsQuery, SalesPermissionDto>
{
    private readonly IIdentityDbContext _db;
    private readonly IClock _clock;
    private readonly AdminOptions _adminOptions;

    public GetSalesPermissionsHandler(IIdentityDbContext db, IClock clock, AdminOptions adminOptions)
    {
        _db = db;
        _clock = clock;
    ...
    public async Task<SalesPermissionDto> Handle(GetSalesPermissionsQuery req, CancellationToken ct)
    {
        ...
        var items = SalesPermissions.All
            .Select(p => new SalesPermissionItemDto(
                p.Key,
                p.Title,
                p.Description,
        ...
        return new SalesPermissionDto(req.UserId, isSuper, uiPolicy, items);
    }
}

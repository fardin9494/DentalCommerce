...
public sealed record GetCatalogPermissionsQuery(Guid UserId) : IRequest<CatalogPermissionDto>;

public sealed class GetCatalogPermissionsHandler : IRequestHandler<GetCatalogPermissionsQuery, CatalogPermissionDto>
{
    private readonly IIdentityDbContext _db;
    private readonly IClock _clock;
    private readonly AdminOptions _adminOptions;

    public GetCatalogPermissionsHandler(IIdentityDbContext db, IClock clock, AdminOptions adminOptions)
    {
        _db = db;
        _clock = clock;
    ...
    public async Task<CatalogPermissionDto> Handle(GetCatalogPermissionsQuery req, CancellationToken ct)
    {
        ...
        var items = CatalogPermissions.All
            .Select(p => new CatalogPermissionItemDto(
                p.Key,
                p.Title,
                p.Description,
        ...
        return new CatalogPermissionDto(req.UserId, isSuper, uiPolicy, items);
    }
}

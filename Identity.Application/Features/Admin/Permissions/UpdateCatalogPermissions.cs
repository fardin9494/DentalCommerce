...
public sealed record UpdateCatalogPermissionsCommand(Guid UserId, UpdateCatalogPermissionsRequest Request) : IRequest;

public sealed class UpdateCatalogPermissionsHandler : IRequestHandler<UpdateCatalogPermissionsCommand>
{
    private readonly IIdentityDbContext _db;
    private readonly IClock _clock;

    public UpdateCatalogPermissionsHandler(IIdentityDbContext db, IClock clock)
    {
        _db = db;
        _clock = clock;
    }

    public async Task Handle(UpdateCatalogPermissionsCommand cmd, CancellationToken ct)
    {
        ...
        {
            if (!CatalogPermissions.IsValid(grant.Key)) continue;

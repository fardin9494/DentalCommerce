...
public sealed record UpdateInventoryPermissionsCommand(Guid UserId, UpdateInventoryPermissionsRequest Request) : IRequest;

public sealed class UpdateInventoryPermissionsHandler : IRequestHandler<UpdateInventoryPermissionsCommand>
{
    private readonly IIdentityDbContext _db;
    private readonly IClock _clock;

    public UpdateInventoryPermissionsHandler(IIdentityDbContext db, IClock clock)
    {
        _db = db;
        _clock = clock;
    }

    public async Task Handle(UpdateInventoryPermissionsCommand cmd, CancellationToken ct)
    {
        ...
        {
            if (!InventoryPermissions.IsValid(grant.Key)) continue;

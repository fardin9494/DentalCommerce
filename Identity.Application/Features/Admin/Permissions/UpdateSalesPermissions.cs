...
public sealed record UpdateSalesPermissionsCommand(Guid UserId, UpdateSalesPermissionsRequest Request) : IRequest;

public sealed class UpdateSalesPermissionsHandler : IRequestHandler<UpdateSalesPermissionsCommand>
{
    private readonly IIdentityDbContext _db;
    private readonly IClock _clock;

    public UpdateSalesPermissionsHandler(IIdentityDbContext db, IClock clock)
    {
        _db = db;
        _clock = clock;
    }

    public async Task Handle(UpdateSalesPermissionsCommand cmd, CancellationToken ct)
    {
        ...
        {
            if (!SalesPermissions.IsValid(grant.Key)) continue;

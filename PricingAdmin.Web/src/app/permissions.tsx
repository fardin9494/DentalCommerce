...
type PricingPermissionsResponse = {
...
export function PricingPermissionsProvider({ children }: PropsWithChildren) {
  const { token } = useAdminAuth()
  const [data, setData] = useState<PricingPermissionsResponse | null>(null)
  ...
  useEffect(() => {
    ...
    setIsLoading(true)
    fetchIdentityJson<PricingPermissionsResponse>('/me/permissions/pricing', { token })
      .then((resp) => {
        if (!isActive) return
        setData(resp)
      })
...
export function usePricingPermissions() {
  const ctx = useContext(PermissionsContext)
  if (!ctx) throw new Error('usePricingPermissions must be used within PricingPermissionsProvider')
  return ctx
}
...
export function PermissionGate({ permission, anyPermissions, policyOverride, children }: PermissionGateProps) {
  const { can, canAny, uiPolicy, isLoading } = usePricingPermissions()
